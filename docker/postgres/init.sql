...
CREATE TABLE IF NOT EXISTS raw.weather_hourly (
    ...
    -- Store name of API
    source_name VARCHAR(50) NOT NULL DEFAULT 'Open-Meteo',
...
CREATE TABLE IF NOT EXISTS metadata.pipeline_run (
   ...
   -- Unique ID for each execution of the data pipeline
    ...
    error_message TEXT,
    CONSTRAINT check_status CHECK (status IN ('success', 'failure', 'running'))
);
...
CREATE INDEX IF NOT EXISTS idx_pipeline_run_started_at ON metadata.pipeline_run (started_at);